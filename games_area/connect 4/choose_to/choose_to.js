// Connect 4 - Mode Selection
console.log('Connect 4 mode selection loaded');
...
function startGame(mode) {
    ...
    localStorage.setItem('connect4GameMode', mode);
    localStorage.setItem('connect4VsAI', mode === 'single' ? 'true' : 'false');
    
    // Store difficulty if vs AI
    if (mode === 'single' && selectedDifficulty) {
        localStorage.setItem('connect4Difficulty', selectedDifficulty);
    } else {
        localStorage.removeItem('connect4Difficulty');
    }
    
    // Redirect to main game
    window.location.href = '../connect_4.html';
}
